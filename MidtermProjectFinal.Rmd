...
ggplot(data=Make_amount, aes(x=reorder(Make,-Count), y=Count)) +
...
```{r, echo=FALSE, fig.height=7, fig.width=14, fig.cap="xxx"}
#### variable "Make" boxplot

# Using median
Make_amount %>%
  mutate(Make=fct_reorder(Make,Count, .fun='median')) %>%
  ggplot(data=Make_amount, aes(x=Make, y=Count, fill=Make)) + 
    geom_boxplot(aes(x=Make, y=Count)) +
    theme(legend.position="none") 

# reorder is close to order, but is made to change the order of the factor levels.
Make_amount$Count <- with(Make_amount, reorder(Make, Count, median))

#p <- mpg %>%
  ggplot(Make_amount, aes(x=Make, y=Count, fill=count)) + 
    geom_boxplot() +
    xlab("class") +
    theme(legend.position="none") +
    xlab("")
...
```{r, echo=FALSE, fig.height=7, fig.width=14, fig.cap="xxx"}
corrplot(Corrplot_final, type="upper", order="hclust", tl.col="black", tl.srt=45)
```
